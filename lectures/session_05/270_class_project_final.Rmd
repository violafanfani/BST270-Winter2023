...
```{r}
load('/Users/violafanfani/Documents/uni-harvard/courses/bst270/BST270-Winter2023/lectures/session_01/midus_data_icpsr_04652_20221222/DS0001/04652-0001-Data.rda') # load MIDUS2 interview/questionnaire data
load('/Users/violafanfani/Documents/uni-harvard/courses/bst270/BST270-Winter2023/lectures/session_01/midus_data_icpsr_29282_20221222/DS0001/29282-0001-Data.rda') # load MIDUS2 biomarker data
df <- inner_join(da29282.0001, da04652.0001, by="M2ID", stringsAsFactors = FALSE) # merge the two dataframes using an inner join
dim(df)
...
df$B1PA24B_new = as.character(df$B1PA24B)
df[is.na(df$B1PA24B),]$B1PA24B_new <- ifelse(df[is.na(df$B1PA24B),]$B1PA24 == "(2) NO", "(0) NO", NA)
dim(df)
```

We lose one participant.

```{r}
# drop rows with missing potential confounders
df <- df %>% drop_na(B1PAGE_M2.x, B1PRSEX.x, B1PF7A, B1PB1, B1STINC1, B4ZB1SLG, B1SCHROX, B1PA24B_new, B1SNEGAF) 
dim(df)
```
...
df[is.na(df$B4H36),]$B4H36 <- ifelse(as.character(df[is.na(df$B4H36),]$B4H33)=="(2) NO", 0, NA)
...
```{r}
#Let's keep a copy of the old df
df_old = df
df = df_old
```
...
3. Is there a codebook and/or instructions about how the data and documentation is organized?
 Yes, but the authors didn't do any of it, and they didn't provide a clear README to guide the readers.
...
7. If the software is available, is it well commented?
  No, but at least they specify the version of the statistical software
8. Is there a toy example provided?
  No
9. Are you able to reproduce the figures, tables and results presented in the paper?
  We could not reproduce the exact figures and tables, we could replicate similar results, with the exception of CHRONIC CONDITIONS.  
10. Was there anything you think should have been made clearer, or explained in a different way?
 The inclusion/ exclusion criteria, variable definition, they do give info about the statistical tests, but not the details of the actual computation
11. Did you find any faults in the methods used in this paper? Would you have used more or different methods?
  Very basic model, could have been developed and explained more in detail.
